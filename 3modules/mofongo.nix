...
let
  ...
  configFile = settingsFormat.generate "mofongo-config.txt" (
    filterAttrs (_n: v: v != null) cfg.settings
  );
in
{
      ...
      type = types.submodule (
        { ... }:
        {
          ...
          options = {
            ...
            songinstatus =
              mkEnableOption "shows the currently playing song in the status. Does not work if it is playing in multiple guilds."
              // {
                default = false;
              };
            altprefix = mkOption {
  ...
  config = mkIf (cfg.enable) {
    users = {
      ...
      groups = optionalAttrs (cfg.group == "mofongo") { mofongo = { }; };
    };
      ...
      services.mofongo = {
        ...
        serviceConfig = {
          ...
          ExecStart = "${cfg.javaPkg}/bin/java -Dnogui=true -Dconfig=\"${
            if (cfg.appendConfigWithFile != null) then "${cfg.stateDir}/config.txt" else configFile
          }\" -jar ${cfg.package}";
          UMask = "0002";
        };
      };
